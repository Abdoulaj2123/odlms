...
Class Master extends DBConnection {
	...
	function save_appointment(){
		if(empty($_POST['id'])){
			...
			$code = sprintf("%'.04d",1);
			while(true){
				$check = $this->conn->query("SELECT * FROM `appointments` where `code` = '{$prefix}{$code}'")->num_rows;
				if($check > 0){
		...
		if(empty($id)){
			$sql = "INSERT INTO `appointments` set {$data} ";
		}else{
			$sql = "UPDATE `appointments` set {$data} where id = '{$id}' ";
		}
		...
		if($save){
			...
			if(isset($test_ids) && count($test_ids) > 0){
				...
				if(!empty($data)){
					...
					if(!$save2){
						...
						$resp['error'] = $this->conn->error;
						if(empty($id))
						$this->conn->query("DELETE * FROM `appointments` where id = '{$aid}'");
						return json_encode($resp);
					}
				}
			}
			...
			if(isset($_FILES['prescription']) && !empty($_FILES['prescription']['tmp_name'])){
				...
				if($move){
					$this->conn->query("UPDATE `appointments` set prescription_path = CONCAT('{$fname}','?v=',unix_timestamp(CURRENT_TIMESTAMP)) where id = '{$aid}'");
				}else{
	...
	function update_appointment_status(){
		...
		$update = $this->conn->query("UPDATE `appointments` set `status` = '{$status}' where id = '{$id}'");
		if($update){
			$resp['status'] = 'success';
			...
			if(isset($_FILES['report'])){
				...
				$get = $this->conn->query("SELECT * FROM appointments where id = '{$id}'");
				$res = $get->fetch_array();
				$code = $res['code'];
				$fname = $code.".pdf";
