...
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT a.*,CONCAT(c.firstname,' ',c.middlename,' ',c.lastname) as fullname, c.contact, c.gender,c.email,c.address FROM `appointments` a inner join `client_list` c on a.client_id = c.id where a.id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
