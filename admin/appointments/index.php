			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
					<?php 
						$i = 1;
						$patient_arr = [];
						$patients = $conn->query("SELECT *,CONCAT(firstname,' ',middlename,' ', lastname) as fullname FROM `client_list` where id in (SELECT client_id FROM `appointments`)");
						if($patients->num_rows > 0){
							$res = $patients->fetch_all(MYSQLI_ASSOC);
							$patient_arr = array_column($res,'fullname','id');
						}
						$qry = $conn->query("SELECT * from `appointments` order by unix_timestamp(date_created) desc ");
